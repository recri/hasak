...
cosine_series3(size, k, a0, a1, a2, a3) = cosine_series2(size, k, a0, a1, a2) - a3 * cos((3 * 2π * k) / (size-1))
# generate hann ramp
hann(k, size) = Integer(round(32767 * cosine_series1(size, k, 0.50, 0.50)))
# generate blackman harris ramp
blackman_harris(k, size) = Integer(round(32767 * cosine_series3(size, k, 0.3587500, 0.4882900, 0.1412800, 0.0116800)))
# generate linear ramp
triangular(k, size) = Integer(round(32767 * (1.0 - abs( (k - ((size-1)/2.0)) / (size/2.0) ))))
# generate sine table
sine(k, size) = Integer(round(32767 * sin(2π * k / size)))
# format tables
using Printf
