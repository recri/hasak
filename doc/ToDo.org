...
[ ] make voices into fists, named KYRF_*
[ ] make notes named KYRN_*
[ ] make all notes and controls and NRPNs listenable, up to
	255 listener channels identified by non-zero byte
[ ] queue events generated during usbMIDI.read() to fire
	during loop time.	
[ ] rewrite the keyer timings to µs
[ ] rewrite the key to arbiter logic using notes.
[x]   inpin.h - digital read pins, debounce, update notes
[x]   note.h - toggle note, send
