...
[ ] use get_nrpnx() for accessing the extended precision nrpn values.
[ ] make nrpn_update_timing notice whether any nrpns are vox special
	so it can write the computed timing to the vox special extended
	storage rather than the default.
[ ] HSMS/QRQ to supersize the speed control, HSMS goes up to 4000 
...
[ ] implement mouse wheel on spinner value change behavior
[ ] implement drag mouse out of spinner value change behvior
